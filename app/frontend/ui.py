...
if st.button("Ask Agent") and user_query.strip():
    ...
    try:
        ...
        #api_url tells where to send the data 
        #payload tells what data to be sent

        if response.status_code==200:
            agent_response = response.json().get("response","") #output form backend is stored in this variable
